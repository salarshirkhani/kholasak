                              ...
                              <div class="col-md-3">
                                  <a href="{{route('post',['id'=>$item->post->id])}}">
                                      <div class="item-main animation" >
                                          <div><img src="{{ asset('pics/'.$item->post->pic.'/'.$item->post->pic ) }}" alt="{{$item->post->title}}"></div>
                                          <div><h3>  
                                            {{$item->post->title}} 
                                          </h3></div>
                                          <div><span>{!! \Illuminate\Support\Str::limit($item->post->explain, 325, ' ...') !!}</span></div>
                                      </div>
                                  </a>
                              </div>
                            @endforeach
              ...
              @foreach ($posts->take(16) as $item)   
                <div class="carousel-cell carousels">
                      <div class="productdesc">
                          <a href="{{route('post',['id'=>$item->post->id])}}">
                              <div class="item-main animation">
                                  <div><img src="{{ asset('pics/'.$item->post->pic.'/'.$item->post->pic ) }}"
                                          alt="{{$item->post->title}}"></div>
                                  <div><span> 
                                    {{$item->post->title}}
                                      </span></div>
                                  <div><span>{!! \Illuminate\Support\Str::limit($item->post->explain, 325, ' ...') !!}</span></div>
